            ...
            try:
                ...
                if score <= 79.99:
                    ...
                    try:
                        ...
                        insertFields = str(nextID) + ", '" + str(score) + "', '" + cleanAddress + "', '" + str(row[1]) + "', '"  + cleanName  + "', '" + titlec + "', '" + str(row[4]) + "', '" + str(row[5]) + "', " + location + ", '" + diocese + "', '" + region + "', '" + congDist + "', '" + str(row[6]) + "', '" + str(row[7]) + "', '" + ccgp + "', '" + grat + "', '" + pac + "', '" + usState + "'"
                        outFields = 'OBJECTID, Score, Address, Id, Name, Title__c, AccountID, Relationship_Types__c, Shape, Diocese, Region, CongressionalDistrict, kw__Chamber__c, kw__StateOfCoverage__c, GrassrootsSupporter, ActionTaker, ParishAmbassador, USState'

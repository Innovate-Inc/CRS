...
## Creates simply ProjectsFC table for US Ops map "Overseas Projects" data
## 
...
with arcpy.da.SearchCursor(fd, fieldNames) as sCursor:
    for row in sCursor:
        ...
        if row[6] is not None:
            projDonor =  'https://catholicreliefservices.my.salesforce.com/' + row[6].encode("utf8").rstrip()
        else:
            projDonor = '-'
        ...
        if row[8] is None:
            ...
            projDonor2 = 'https://catholicreliefservices.my.salesforce.com/' + row[8].encode("utf8").rstrip()

        # Project Donor 3
        if row[9] is not None:
            projDonor3 = 'https://catholicreliefservices.my.salesforce.com/' + row[9].encode("utf8").rstrip()
        else:
            projDonor3 = '-'    

        # Project Donor 4
        if row[10] is not None:
            projDonor4 = 'https://catholicreliefservices.my.salesforce.com/' + row[10].encode("utf8").rstrip()
        else:
            projDonor4 = '-'

        # Project Donor 5
        if row[11] is not None:
            projDonor5 = 'https://catholicreliefservices.my.salesforce.com/' + row[11].encode("utf8").rstrip()
        else:
            projDonor5 = '-'
        ...
        if row[2] is None:
            ...
            for country in countryList:
                ...
                sqlString = "Use Salesforce_Data Declare @g geometry; Set @g = (SELECT poly.Shape FROM countriesAGOL as poly where poly.Country = '" + country + "') Insert into ProjectsFC(" + outFields + ") values (" + insertFields + ", @g)"
                print "Added project to " + country
